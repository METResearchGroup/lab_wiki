...
- (Backend) [Railway](https://railway.com/): Railway hosts the part of the app that runs in the background. You deploy a server, a database, or a worker, and Railway keeps that process running and gives you logs, env vars, and a URL for the API. To access Railway, see the [HOW_TO_ACCESS_RAILWAY.md](../tools/webapp/HOW_TO_ACCESS_RAILWAY.md) docs.
